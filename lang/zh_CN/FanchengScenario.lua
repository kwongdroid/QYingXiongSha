-- translation for FanchengScenario

return {
	["fancheng"] = "荆轲刺秦",
	["guagu"] = "治国",
	["flood"] = "焚书坑儒",
	["taichen"] = "刺秦",
	["changqu"] = "退避三舍",
	["taichen_fight"] = "图穷匕见",
	["taichenfight"] = "图穷匕见",
	["zhiyuan"] = "浣纱",
	[":guagu"] = "锁定技：商鞅每对嬴政造成1点伤害，嬴政回复2点体力，且商鞅再摸一张牌",
	[":dujiang"] = "限定技：在吕蒙的回合开始阶段，他可弃掉2张已装备的装备，变身为神吕蒙，体力上限变为3（若当前体力值为4，\
则同样降到3），失去原来的克己技能，同时获得神吕蒙的涉猎与攻心的技能。若回合开始时已装备的装备数不足2，\
则无法发动此技能。若水淹七军剧情已发生，则曹仁失去的手牌上限-1的不利效果",
	[":flood"] = "限定技：嬴政可弃3张黑色手牌，令所有反贼弃掉所有装备，并弃2张手牌或受到嬴政的1点伤害，弃手牌还是装备由\
反贼决定，若手牌不足两张则必须受到伤害（与复仇成功一样），西施的手牌上限-1",
	[":taichen"] = "出牌阶段，可自减1点体力，视作对嬴政打出了一张决斗，此决斗不能被无懈可击，无论嬴政杀死荆轲无\
3张牌的奖励，即视为天灾",
	[":changqu"] = "锁定技：焚书坑儒剧情发生后，廉颇到嬴政的距离为1，但反之嬴政到廉颇的距离正常计算",
	[":xiansheng"] = "限定技：在关羽回合开始阶段，若体力小于等于2，则可以弃掉所有装备和手牌，然后摸3张手牌，变身为神关羽（没\
有武魂的索命技能，仅有武神），体力和体力上限皆为6。抬榇战关羽、长驱直入、刮骨疗毒都能对新的神关羽起作用",
	[":zhiyuan"] = "你可在出牌阶段给荆轲或廉颇一张基本牌，每回合限用2次",
	["@@dujiang"] = "请弃掉两张已装备的装备牌，变身为神吕蒙",
}
