...
function guihan_skill.getTurnUseCard(self)
	...
	if mode:find("04_1v3") or mode == "06_3v3" then return end
	...
	for index = #cards, 1, -1 do
		...
		if cards[index]:isBlack() then
			if #black_cards == 0 or (#black_cards == 1 and cards[index]:getSuit() == sgs.Sanguosha:getCard(black_cards[1]):getSuit()) then
	...
	if #black_cards == 2 then return sgs.Card_Parse("@GuihanCard=" .. table.concat(black_cards, "+")) end
end
