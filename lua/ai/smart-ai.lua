...
version = "QSanguosha AI 20130801 (V0.1 Stable)"
...
function setInitialTables()
	...
	sgs.masochism_skill = 		"fankui|jieming|yiji|ganglie|enyuan|fangzhu|guixin|wange|kuidao|shixin|wudao|yaoyi"
	sgs.wizard_skill = 			"guicai|guidao|jilve|kuixin"
	sgs.wizard_harm_skill = 	"guicai|guidao|jilve"
	sgs.priority_skill = 		"dimeng|haoshi|qingnang|jizhi|guzheng|sheji|qixi|jieyin|guose|duanliang|qimen|fenghuo|duizhi|miaobi|jianai|shipo|jujia"
	...
	sgs.cardneed_skill =        "zhongwu|jianai|miaobi|qimen|shentou|conglang|paoxiao|tianyi|sanbanfu|budao|shuangxiong|jizhi|guose|duanliang|qixi|qingnang|" ..
								"jieyin|shipo|zhiheng|rende|jujian|guicai|guidao|shentan|longhun|wusheng|longdan"
...
function sgs.getDefense(player)
	...
	if player:getArmor() and player:getArmor():inherits("EightDiagram") and player:hasSkill("kuixin") then
		defense = defense + 0.5
	end
	if player:hasSkill("jieming") or player:hasSkill("yiji") or player:hasSkill("kuidao") then
		defense = defense + 4
	end
	...
	if player:hasSkill("xiadao") and player:getHandcardNum()>1 and player:getEquips():isEmpty() then
		defense = defense + 0.5
	end
...
function SmartAI:getUseValue(card)
	...
	if card:getTypeId() == sgs.Card_Equip then
		...
		if self:hasSkill("piaoqi") and card:inherits("Weapon") then v = 2 end
	...
	elseif card:getTypeId() == sgs.Card_Basic then
		if card:inherits("Slash") then
			if (self.player:hasFlag("drank") or self.player:hasFlag("tianyi_success") or self.player:hasFlag("luoyi")) or self.player:hasSkill("zhongwu") or self.player:hasSkill("xlongdan") then v = 8.7 end
		...
		elseif card:inherits("Peach") then
			if self.player:hasSkill("jianxiongyxs") and self.player:getLostHp() >= 2 then v = v + 5 end
	...
	elseif card:getTypeId() == sgs.Card_Trick then
		...
		if self.player:hasSkill("conglang") then v = 8.7 end
		if self.player:hasSkill("shentou") and card:inherits("Snatch") then v = 8.7 end
	...
	if self:hasSkill("halberd") and card:inherits("Slash") and self.player:getMark("@yuxian1") <= 0 and self.player:getHandcardNum() == 1 then v = 10 end
...
function SmartAI:getUsePriority(card)
	...
	if self.player:hasSkill("shentou") then
		if card:inherits("Snatch") then v = 4.5 end
		if v then return v else return sgs.ai_use_priority[class_name] end
	end
	if self.player:hasSkill("jianxiongyxs") and self.player:getLostHp() >= 2 then
		if card:inherits("Peach") then v = 9 end
		if v then return v else return sgs.ai_use_priority[class_name] end
	end
	if self.player:hasSkill("zhongwu") and self.player:getLostHp() >= 1 then
		if card:inherits("Slash") then v = 3 end
		if v then return v else return sgs.ai_use_priority[class_name] end
	end
	if self.player:hasSkill("shentou") then
		if card:inherits("Snatch") then v = 4.5 end
...
function SmartAI:cardNeed(card)
	...
	local suit_string = card:getSuitString()
	local value
	if card:inherits("Peach") or card:inherits("Fuzhou") then
	...
	if card:inherits("Crossbow") and  self:hasSkills("luoshen|yongsi|kurou|keji|wusheng|zhongwu",self.player) then return 20 end
...
function sgs.isRolePredictable()
	...
	local mode = string.lower(global_room:getMode())
	if not mode:find("0") or mode:find("03p") or mode:find("02_1v1") or mode:find("04_1v3") or mode == "06_3v3" or mode == "06_xiuxian"or mode:find("mini") then return true end
	return false
end
...
sgs.ai_choicemade_filter.Nullification.general = function(player, promptlist)
	...
	elseif (className == "Snatch" or className == "Dismantlement") and
		...
		not (to:getArmor() and (to:getArmor():inherits("GaleShell") or to:getArmor():inherits("SilverLion") or to:getArmor():inherits("Jinlinjia")))) then
		intention = 80
	end
	...
	elseif event == sgs.CardLost then
		...
		if sgs.ai_snat_disma_effect then
			...
			elseif place == sgs.Player_Equip then
				if player:getLostHp() > 1 and (card:inherits("SilverLion") or card:inherits("Jinlinjia")) then intention = -intention end
...
function SmartAI:askForDiscard(reason, discard_num, optional, include_equip)
	...
	local aux_func = function(card)
		...
		if place == sgs.Player_Equip then
			...
			elseif card:inherits("Qiankundai") then return -2
			elseif (card:inherits("SilverLion") or card:inherits("Jinlinjia")) and self.player:isWounded() then return -2
...
function SmartAI:askForNullification(trick, from, to, positive)
	...
	if positive then
		if from and self:isEnemy(from) and (sgs.evaluateRoleTrends(from) ~= "neutral" or sgs.isRolePredictable()) then
			...
			if (trick:inherits("Geanguanhuo") or trick:inherits("Toulianghuanzhu")) and self:isFriend(to) then return null_card end
...
function SmartAI:getCardRandomly(who, flags)
	...
	if (self:isEquip("SilverLion", who) or self:isEquip("Jinlinjia", who)) then
...
function SmartAI:askForCardChosen(who, flags, reason)
	...
	if self:isFriend(who) then
		...
		if flags:match("e") then
			...
			if who:isWounded() and (self:isEquip("SilverLion", who) or self:isEquip("Jinlinjia", who)) and (not zhangjiao or self:isFriend(zhangjiao))
				and not self:hasSkills("qixi|duanliang", who) then return who:getArmor():getId() end
			if self:evaluateArmor(who:getArmor(), who)<-5 and not self:isEquip("Qiankundai", who) then return who:getArmor():getId() end
		...
		if flags:match("e") then
			...
			if who:getArmor() and not who:getArmor():inherits("Qiankundai") and self:evaluateArmor(who:getArmor(),who)>3 then
		...
		if flags:match("e") then
			if who:getArmor() and self:evaluateArmor(who:getArmor(), who)>0
				and not (who:getArmor():inherits("SilverLion") and self:isWeak(who)) and not (who:getArmor():inherits("Jinlinjia") and self:isWeak(who)) and not who:getArmor():inherits("Qiankundai") then
				return who:getArmor():getId()
			end
...
function SmartAI:askForCard(pattern, prompt, data)
	...
	if pattern == "slash" then
		return sgs.ai_skill_cardask.nullfilter(self, data, pattern, target) or self:getCardId("Slash") or self:getCardId("Fuzhou")  or "."
	elseif pattern == "jink" then
		return sgs.ai_skill_cardask.nullfilter(self, data, pattern, target) or self:getCardId("Jink") or self:getCardId("Fuzhou") or "."
	end
end
	...
	if #self.friends > 1 then
		for _, hcard in ipairs(cards) do
			if not hcard:inherits("Shit") then
				if hcard:inherits("Analeptic") or hcard:inherits("Peach") or hcard:inherits("Fuzhou") then
...
function SmartAI:askForYiji(card_ids)
	if self.player:getHandcardNum() <= 2 and self.player:getHp() > 0 then
		return nil, -1
	end
	...
	if #self.friends > 1 and (self:getOverflow() > 0 or self.player:getHp() <= 0) then
...
function SmartAI:askForSinglePeach(dying)
	...
	if self:isFriend(dying) then
		...
		if (self.player:objectName() == dying:objectName()) then
			card_str = self:getCardId("Analeptic") or self:getCardId("Peach") or self:getCardId("Fuzhou")
		elseif dying:isLord() then
			card_str = self:getCardId("Peach") or self:getCardId("Fuzhou")
		else
			for _, friend in ipairs(self.friends_noself) do
...
function SmartAI:getTurnUse()
	...
	if self.player:hasFlag("fanwei") then
		self.slash_distance_limit = true
	end

	if self.player:hasSkill("sheshu") then
		self.slash_distance_limit = true
	end

	if self.player:hasFlag("ewslash") then
		slashAvail = slashAvail+1
	end

	if self.player:hasSkill("zhongwu") then
		slashAvail = slashAvail+self.player:getLostHp()
	end

	if self.player:hasFlag("shuang") then
		self.slash_targets = self.slash_targets+1
	end
	...
	if self:isEquip("Crossbow") and self.player:getMark("@yuxian1") <= 0 then
		slashAvail = 100
	end
...
function SmartAI:damageIsEffective(player, nature, source)
	...
	if (player:hasSkill("zhichi") and self.room:getTag("Zhichi"):toString() == player:objectName()) or (player:hasSkill("chajue") and self.room:getTag("Zhichi"):toString() == player:objectName()) then
		return false
	end
...
function SmartAI:aoeIsEffective(card, to)
	...
	if armor and armor:inherits("Jinlinjia") then
		return false
	end
	...
	if ((to:hasSkill("zhichi") and self.room:getTag("Zhichi"):toString() == to:objectName())) or ((to:hasSkill("chajue") and self.room:getTag("Zhichi"):toString() == to:objectName())) then
		return false
	end

	if to:hasSkill("diehun") and #players == 2 then
		return false
	end

	if not to:hasSkill("diehun") and #players == 2 and not self.room:isProhibited(self.player, to, card) then
		return true
	end
...
function SmartAI:getDistanceLimit(card)
	if card:inherits("Snatch") and self.player:hasSkill("feizei") then
		return 100
	end

	if card:inherits("Snatch") and not self.player:hasSkill("feizei") then
		return 1
...
function SmartAI:getAoeValueTo(card, to , from)
	...
	if self:aoeIsEffective(card, to) then
		...
		if card:inherits("ArcheryAttack") then
			...
			if (to:hasSkill("xiadao") and to:getEquips():isEmpty()) or to:hasSkill("qingguo") or self:isEquip("EightDiagram", to) then
				value = value + 20
			end
		end
...
function SmartAI:hasTrickEffective(card, player)
	if player then
		...
		if player:hasSkill("yinju") and player:getMark("@yinju1") > 0 and card:isRed() and card:inherits("Duel") then return false end
		if player:hasSkill("yinju") and player:getMark("@yinju2") > 0 and card:isBlack() and card:inherits("Duel") then return false end
		if player:hasSkill("hongmian") and player:getMark("@qianfu") > 0 and card:inherits("Duel") then return false end
		if card:inherits("TrickCard") and player:hasSkill("xinjujian") and self.player:getMark("@jujian") > 0 then return false end
		if ((player:hasSkill("zhichi") and self.room:getTag("Zhichi"):toString() == player:objectName())) or ((player:hasSkill("chajue") and self.room:getTag("Zhichi"):toString() == player:objectName())) or player:hasSkill("wuyan") then
...
function SmartAI:useTrickCard(card, use)
	...
	if card:inherits("AOE") then
		...
		if self.role == "renegade" or self:hasSkill("jianxiong|luanji|fenghuo",self.player) then
...
function SmartAI:useEquipCard(card, use)
    if not self.player:getArmor() and card:inherits("Qiankundai") and not self.player:hasSkill("bazhen") then
		use.card = card
		return
	end
	if self:isEquip("Qiankundai") and card:inherits("Armor") then
		use.card = card
		return
	end
	...
	elseif card:inherits("Armor") then
		...
		local lion = self:getCard("Jinlinjia")
		if lion and self.player:isWounded() and not self:isEquip("Jinlinjia") and not card:inherits("Jinlinjia") and
		...
		for _, acard in ipairs(cards) do
			...
			elseif acard:inherits("Slash") and self:slashIsEffective(acard, enemy) and ( slash_damagenum == 0 or self:isEquip("Crossbow", self.player))
				...
				if self:getCardsNum("Analeptic") > 0 and analepticpowerup == 0 and
					not ((self:isEquip("Huxinjing", enemy) or self:isEquip("EightDiagram", enemy) or
				...
				if self:isEquip("GudingBlade", self.player) and enemy:isKongcheng() and not self:isEquip("SilverLion", enemy) and not self:isEquip("Huxinjing", enemy) then
					slash_damagenum = slash_damagenum + 1
				end
			end
		end
...
dofile "lua/ai/yjcm2012-ai.lua"
dofile "lua/ai/fancheng-ai.lua"
dofile "lua/ai/guanxing-ai.lua"
dofile "lua/ai/special3v3-ai.lua"
dofile "lua/ai/special-ai.lua"
dofile "lua/ai/yxsyjcm-ai.lua"
dofile "lua/ai/shijie-ai.lua"
dofile "lua/ai/zombie_mode-ai.lua"
dofile "lua/ai/joy-ai.lua"
dofile "lua/ai/bgm-ai.lua"
dofile "lua/ai/douhun-ai.lua"
dofile "lua/ai/kylin-ai.lua"
dofile "lua/ai/threekingdoms-ai.lua"
dofile "lua/ai/yxs-ai.lua"
