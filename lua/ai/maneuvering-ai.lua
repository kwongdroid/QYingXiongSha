...
function SmartAI:searchForAnaleptic(use,enemy,slash)
	...
	if self.player:hasUsed("Analeptic") or self.player:getHp() <= 1 or self:isWeak() or self.player:getMark("kongqueling") >= 1 then return nil end
...
function SmartAI:useCardSupplyShortage(card, use)
	...
	local man = self.player:getNextAlive()
		...
		if self:isEnemy(man) and not man:containsTrick("supply_shortage") and man:faceUp() and self.player:distanceTo(man) <= 1 and not self.room:isProhibited(self.player, man, card) then
			use.card = card
	...
	for _, enemy in ipairs(enemies) do
		if (self:hasSkills("yongsi|haoshi|tuxi", enemy) or (enemy:hasSkill("zaiqi") and enemy:getLostHp() > 1)) and
			not enemy:containsTrick("supply_shortage") and enemy:faceUp() and self:objectiveLevel(enemy) > 3 and self.player:distanceTo(enemy) <= 1 then
			use.card = card
...
function SmartAI:useCardIronChain(card, use)
	...
	local friendtargets = {}
	local enemytargets = {}
	local yangxiu = self.room:findPlayerBySkillName("diehun")
	self:sort(self.friends,"defense")
	for _, friend in ipairs(self.friends) do
		if friend:isChained() and not self:isGoodChainPartner(friend) and self:hasTrickEffective(card, friend) and not friend:hasSkill("diehun") then
			table.insert(friendtargets, friend)
		end
	end
	self:sort(self.enemies,"defense")
	for _, enemy in ipairs(self.enemies) do
		if not enemy:isChained() and not self.room:isProhibited(self.player, enemy, card) and not enemy:hasSkill("diehun")
