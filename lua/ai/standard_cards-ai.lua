...
function SmartAI:slashIsEffective(slash, to)
	...
	local mozi = self.room:findPlayerBySkillName("feigong")
	if mozi and to:getMark("@feigong2") > 0  then return false end

	if self.player:getMark("@feigong1") > 0 and to:getMark("@feigong3") > 0 then return false end

	if self.player:getMark("gedi") > 0 and to:hasSkill("gedi") then return false end

	if to:hasSkill("meiguo") and to:getHp() <= 1 then return false end
		
	if to:hasSkill("#huoshen1") and to:getHp() == 2  then return false end
	
	if to:hasArmorEffect("fenglaiqin") then return false end
	
	if to:hasSkill("#qinglianbuff2") and to:getMark("@qinglianbuff2")>0 then return false end
	...
	if  to:getMark("@tongling2") > 0 then return false end

	if to:hasSkill("hongmian") and to:getMark("@qianfu") > 0 then
		if slash:getSuit() == sgs.Card_Club then
			return false
		end
	end

	if to:hasSkill("fanpu") and to:getMark("@tongling2") > 0 then return false end

	if to:hasSkill("heimian") and not to:getArmor() then
		if slash:getSuit() == sgs.Card_Spade then
			return false
		end
	end

	if to:hasSkill("xianqu") then
		if slash:getNumber() < 8 then
			return false
		end
	end

	if to:hasSkill("yinju") and to:getMark("@yinju1") > 0 then
		if slash:isRed() then
			return false
		end
	end

	if to:hasSkill("yinju") and to:getMark("@yinju2") > 0 then
		if slash:isBlack() then
			return false
		end
	end
	...
	local natures = {
		Slash = sgs.DamageStruct_Thunder,
		XueSlash = sgs.DamageStruct_Fire,
		AnSlash = sgs.DamageStruct_Thunder,
	}
	...
	if self.player:hasSkill("xchongfeng") and to:hasFlag("ChongfengTarget") then
		return slash:inherits("XueSlash") or slash:inherits("AnSlash") or slash:inherits("Slash")
	end

	if self:hasSkills(sgs.masochism_skill, to) and self:getCardsNum("AnSlash") > 0 then
		return slash:inherits("AnSlash")
	end

	if self.player:getLostHp() > 0 and self:getCardsNum("XueSlash") > 0 then
		return slash:inherits("XueSlash")
	end
...
function SmartAI:useCardSlash(card, use)
	...
	if card:isRed() and self.player:hasSkill("xiedao") then no_distance = true end
	...
	for _, target in ipairs(targets) do
		...
		not (not self:isWeak(target) and #self.enemies > 1 and #self.friends > 1 and self.player:hasSkill("keji")
			...
			if not use.to or use.to:isEmpty() then
				...
				if anal and not self:isEquip("SilverLion", target) and not target:hasSkill("yexing") and not target:hasSkill("yinglie") and not self:isWeak() then
	...
	for _, friend in ipairs(self.friends_noself) do
		if friend:hasSkill("yiji") and friend:getLostHp() < 1 and
			...
			if not slash_prohibit then
				if ((self.player:canSlash(friend, not no_distance)) or
					(use.isDummy and (self.player:distanceTo(friend) <= self.predictedRange))) and
					self:slashIsEffective(card, friend) and not card:inherits("AnSlash") then
					use.card = card
					if use.to then
						use.to:append(friend)
						self:speak("yiji")
...
spear_skill.getTurnUseCard=function(self,inclusive)
	...
	if self.player:getMark("@yuxian1") > 0 then return nil end
...
function SmartAI:getValuableCard(who)
	...
	for _,equip in ipairs(equips) do
		...
		if who:hasSkill("guicai") then  return equip:getEffectiveId() end
	...
	if armor and self:evaluateArmor(armor, who)>0
		and not (armor:inherits("SilverLion") and who:isWounded()) and not (armor:inherits("Jinlinjia") and who:isWounded()) then
		return armor:getEffectiveId()
	end
...
function SmartAI:useCardSnatchOrDismantlement(card, use)
	...
	for _, friend in ipairs(friends) do
		...
		if (self:isEquip("SilverLion", friend) or self:isEquip("Jinlinjia", friend)) and self:hasTrickEffective(card, friend) and
...
function SmartAI:useCardCollateral(card, use)
	...
	for _, friend in ipairs(self.friends_noself) do
		if friend:getWeapon() and not friend:hasSkill("weimu") and self:hasSkills(sgs.lose_equip_skill, friend)
...
function SmartAI:useCardIndulgence(card, use)
	...
	local enemies = self:exclude(self.enemies, card)
	local man = self.player:getNextAlive()

		if self:isEnemy(man) and not man:containsTrick("indulgence") and man:faceUp() and not man:hasSkill("keji") and not self.room:isProhibited(self.player, man, card) then
			use.card = card
			if use.to then use.to:append(man) end
			return
	end
