...
sgs.ai_use_priority.RendeCard = 6.5
...
sgs.zhaokuangyin_suit_value =
{
	spade = 3.9,
	club = 3.9
}

sgs.ai_chaofeng.zhaokuangyin = 2
...
kurou_skill.getTurnUseCard=function(self,inclusive)
	...
	if self.player:getWeapon() and self.player:getWeapon():inherits("Crossbow") and self.player:getMark("@yuxian1") <= 0 then
		for _, enemy in ipairs(self.enemies) do
...
sgs.xishi_suit_value =
{
	diamond = 3.9
}

sgs.ai_chaofeng.xishi = 2

sgs.ai_chaofeng.zhuyuanzhang = -1
...
sgs.bianque_suit_value =
{
	heart = 6,
	diamond = 6
}

sgs.ai_chaofeng.bianque = 6
...
sgs.ai_chaofeng.chenyuanyuan = 4
