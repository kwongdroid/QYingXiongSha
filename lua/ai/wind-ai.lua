...
function sgs.ai_skill_invoke.jushou(self, data)
	...
	for _, friend in ipairs(self.friends) do
		if self:hasSkills("xinzhiheng", friend) then return true end
	end
	return self:isWeak()
end
...
function sgs.ai_slash_prohibit.leiji(self, to, card)
    if self:isFriend(to) and (self.player:hasSkill("liegong") or self.player:hasSkill("hanbei") or self.player:hasSkill("jingzhun")) then return true end
